...
@app.get("/api/terrain/sessions/latest")
async def terrain_session_latest() -> Response:
    """Return the metadata of the most-recently registered terrain session.

    Used by the interactive interface on startup so a session pre-generated by
    ``salus interface --scenario`` (D-473) can be picked up by the client
    without the user having to re-upload the same DEM. Returns the same JSON
    shape as ``POST /api/terrain/load`` (session_id, bounds_wgs84, centre_wgs84,
    tile_url_template, tile_progress_url, terrain_min/max_zoom, …) or 404 when
    no usable session is currently registered.

    D-476: a session whose tile-generation thread has reported an error is
    treated as 404 — the client must not adopt a known-broken session and
    request tiles that will 404 indefinitely. A still-generating session
    (``done`` False, ``error`` None) IS advertised: tiles are served as they
    appear on disk and the client can still poll tile-progress.
    """
    with _terrain_session_lock:
        session_id = _terrain_latest_session_id
        session = _terrain_sessions.get(session_id) if session_id else None
        if session is None or session.get("error") is not None:
            return Response(status_code=404)
